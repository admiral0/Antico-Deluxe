...
class AmeSystemSound::SoundOutput : public QThread
{
//	Q_OBJECT
public:
...
AmeSystemSound::SoundOutput::SoundOutput(bool withMixer, QObject *parent)
	: QThread(parent),
	  r(stackSize()),
	  useMixer(withMixer)
{
...
AmeSystemSound::SoundOutput::~SoundOutput()
{
	uninitialize();
}

void AmeSystemSound::SoundOutput::readSettings()
{
	uninitialize();
...
void AmeSystemSound::SoundOutput::uninitialize()
{
	if (pcm_name) {
		free(pcm_name);
		pcm_name = 0;
...
bool AmeSystemSound::SoundOutput::initialize()
{
...
void AmeSystemSound::SoundOutput::run()
{
	mutex()->lock ();
	if (! m_inited) {
		mutex()->unlock();
...
long AmeSystemSound::SoundOutput::alsaWrite(unsigned char *data, long size)
{
	long m = 0;
	if (m_use_mmap)
...
int AmeSystemSound::SoundOutput::setupMixer(QString card, QString device)
{
    char *name;
    long int a, b;
...
void AmeSystemSound::SoundOutput::parseMixerName(char *str, char **name, int *index)
{
	char *end;
...
snd_mixer_elem_t* AmeSystemSound::SoundOutput::getMixerElem(snd_mixer_t *mixer, char *name, int index)
{
...
void AmeSystemSound::SoundOutput::setVolume(int l, int r)
{
	if (!pcm_element)
		return;
...
void AmeSystemSound::SoundOutput::volume(int *l, int *r)
{
	if (!pcm_element)
		return;
...
int AmeSystemSound::SoundOutput::getMixer(snd_mixer_t **mixer, QString card)
{
    char *dev;
    int err;
